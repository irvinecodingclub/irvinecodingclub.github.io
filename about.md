...
<script>
  ...
  return str.replace(
	...
	// document.querySelector('#bio iframe').src = 'https://irvinecoding.repl.co/api/v1/bio/display?name=' + encodeURIComponent(name.toLowerCase().split(' ').join(''));
	fetch('https://irvinecoding.repl.co/api/v1/bio?name=' + encodeURIComponent(name)).then(bio => bio.json()).then(bio => {
            console.log(bio);
            bioOpen = true;
